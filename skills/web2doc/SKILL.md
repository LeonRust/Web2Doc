...
没有 `web2doc` 就装预编译二进制（无需 Rust，自动识别平台）。按系统执行其一：

Linux / macOS（bash）：

```bash
command -v web2doc >/dev/null 2>&1 || \
  curl -fsSL https://raw.githubusercontent.com/LeonRust/Web2Doc/main/scripts/install.sh | bash
# 若装到 ~/.local/bin 但命令仍找不到：export PATH="$HOME/.local/bin:$PATH"
```

Windows（PowerShell）：

```powershell
if (-not (Get-Command web2doc -ErrorAction SilentlyContinue)) {
  irm https://raw.githubusercontent.com/LeonRust/Web2Doc/main/scripts/install.ps1 | iex
}
```

> 仅当没有合适的预编译版时，才回退源码安装（需 Rust 1.85+）：
> `cargo install --git https://github.com/LeonRust/Web2Doc`
