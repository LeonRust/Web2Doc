...
//!
//! M1 注（tasks R-5）：robots 扣除在 M5 接入，本阶段 `baseline_total` 为临时口径。
...
/// 由候选 URL 构建基准全集 + 任务队列：前缀 ∩ 文档页，去重，截断，批量映射，nav_order。
...
mod tests {
    ...
    fn builds_with_filter_dedup_and_truncate() {
        ...
        let d = build_discovery(cands, "x.com", &["/docs/".to_string()], 10);
        ...
        let many: Vec<Url> = (0..5)
            ...
            .collect();
        let d2 = build_discovery(many, "x.com", &["/docs/".to_string()], 3);
