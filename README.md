...
- **正文去噪**：readability 算法提取正文，剔除导航 / 侧栏 / 页脚 / 广告等噪声。
...
- **可信口径**：覆盖率 / 失败率独立度量；`max-pages` 截断标记 Partial 且不判失败。
...
- ✅ **M1** 静态 / SSR 端到端 · **M3** 图片本地化 + bundle · **M5** robots + 口径收尾
- 🚧 **M2** LLM 规则分析（`--base-url` / `--model` + 环境变量 `OPENAI_API_KEY`，提升异构站点去噪准确性）
- 🚧 **M4** 动态引擎（`--mode dynamic`，chromiumoxide 渲染 SPA）

> 注：`--mode` / `--base-url` / `--model` / `--chrome-path` 选项已预留，对应功能在 M2 / M4 里程碑接入。
...
规格文档（SDD）见 `constitution.md` 与 `docs/specs/web2doc/`。
